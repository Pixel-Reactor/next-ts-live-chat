...
export default function SideBar({}) {
  ...
  const [allchannels, setAllchannels] = useState<
    { save_id: string; channel_id: string; channel_name: string }[] | null
  >(null);
  const [popular, setPopular] = useState<
    { quantity: number; channel_id: string; channel_name: string }[] | null
  >(null);
    ...
    >
      ...
      >
        <div className="flex flex-col bg-zinc-900 min-h-[180px] p-2 ">
          <div className="flex justify-between   p-3 z-50 text-left ">
            <h1 className=" w-full p-1 text-left  bg-gradient-to-b from-neutral-100 via-gray-200 to-neutral-900 bg-clip-text text-transparent font-bold text-xl ">
              All Channels
            </h1>
            <span
        ...
        className="flex relative  flex-col gap-2 text-zinc-50  ">
          ...
          <h2 className="font-medium flex items-center gap-3 text-md p-1">
            <span>
          ...
          <div className="p-2 flex flex-col gap-1.5">
            {popular?.map((item) => (
              <div key={item.channel_id} className="relative ">
                <div className="relative flex w-full  hover:bg-zinc-300/10  items-center justify-bewtween  rounded-sm text-slate-300">
                  <p
                    className={`w-full ${
                      channelIn?.id === item.channel_id
                        ? "bg-zinc-800"
                        : "bg-transparent" ?? ""
                    }  font-normal  text-md flex items-center gap-2`}
                  >
                    <PiPlugsConnectedFill size={25} />
                    {item.channel_name}
                  </p>
          ...
          <h2 className="font-medium flex items-center gap-3 text-lg p-2">
            <span>
          ...
          <div className=" h-72 overflow-y-scroll  custom-scrollbar">
            {allchannels?.map((item) => (
              <div key={item.channel_id} className="relative ">
                <div className="relative flex w-full px-1 hover:bg-zinc-300/10  items-center justify-bewtween  rounded-sm text-slate-300">
                  <p
            ...
            {signedUser && signedUser.avatar && (
              <div
                className={`text-zinc-50 ${
                  info ? "hidden" : "flex"
                } h-[300px] border-1 flex flex-col justify-between p-2 rounded-lg border-zinc-300/30 bg-zinc-950/90 w-full  absolute top-[-300px] left-0 z-50 overflow-hidden`}
              >
                <div
